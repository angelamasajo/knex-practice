...
describe(`Articles service object`, function() {
  ...
  //GIVEN THE TABLE HAS NO DATA
  context(`Given 'blogful_articles' has no data`, () => {
    it(`getAllArticles() resoles an empty array`, () => {
      return ArticlesService.getAllArticles(db)
        .then(actual => {
          expect(actual).to.eql([])
        })
    ...
    it(`insertArticle() inserts a new article and resolves the new article with an 'id'`, () => {
      ...
      //attempt to insert new article, injecting Knex instance and passing new article object
      return ArticlesService.insertArticle(db, newArticle)
        //assert that method resolves newly created article with an incremented ID
        //ID should be '1' as the table is empty.
        .then(actual => {
          expect(actual).to.eql({
            id: 1,
